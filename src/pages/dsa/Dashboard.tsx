...
export function DSADashboard() {
  ...
  const fetchDashboardData = async (userId: string) => {
    ...
    try {
      ...
      const { data: commissions } = await supabase.from('commissions').select('amount').eq('dsa_id', userId).eq('status', 'pending')

      if (orders) {
        const deliveredOrders = orders.filter(o => o.status === 'delivered')
        const sales = deliveredOrders.reduce((sum, o) => sum + Number(o.total_amount), 0)
        setStats(prev => ({ ...prev, totalSales: sales }))
        ...
        setCommissionStatus({ camerasDelivered: currentMonthDelivered, target: 30 })
      ...
      if (commissions) {
        const pending = commissions.reduce((sum, c) => sum + Number(c.amount), 0)
        setStats(prev => ({ ...prev, pendingCommissions: pending }))
      }
